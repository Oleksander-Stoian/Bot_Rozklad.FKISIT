...
async def current(msg: types.Message):
    ...
    if found:
        ...
        next_lesson_start = None  # окрема змінна, не залежить від циклу
            ...
            try:
                ...
                if ls > now:
                    ...
                    if subj:
                        ...
                        next_lesson_start = ls
                        break
            except Exception:
                continue

        if next_lesson is not None and next_lesson_start is not None:
            delta_minutes = int((next_lesson_start - now).total_seconds() / 60)
            ...
            await msg.answer(
                f"🎉 <b>На сьогодні все!</b> ({week_label(w_type)}) {header_info}\nБільше пар немає. Відпочивайте!{ERROR_FOOTER}",
                parse_mode="HTML"
            )
...
async def generate_day_schedule_text(day_en, role, groups, teach, w_type):
    ...
    # Передаємо day одразу в filter_schedule — не завантажуємо весь розклад
    ...
    d_df = df.sort_values("Час")  # df вже відфільтровано по дню в filter_schedule
    if d_df.empty:
...
async def process_wkday(cb: types.CallbackQuery):
    ...
    try:
        ...
        pass  # Захист від подвійного кліку на той самий день
